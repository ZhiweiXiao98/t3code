...
import { localizeMobileString, MOBILE_LOCALE } from "../../i18n/mobileStrings";
...
export function UsageRouteScreen() {
  ...
  const filterAccessibilityLabel = localizeMobileString(
  ...
  const environmentActions = useMemo(
    () => [
      {
        id: "all",
        title: localizeMobileString("All environments"),
        subtitle: undefined,
      ...
      ...environments.map((environment) => ({
        ...
        subtitle: localizeMobileString(usageEnvironmentStatus(environment)),
        state:
              ...
              {merged.duplicateSources.length > 0 ? (
                <Text className="text-sm text-foreground-muted">
                  {localizeMobileString(
                    `Counted once across environments sharing a transcript directory: ${merged.duplicateSources.join(", ")}`,
                  )}
                </Text>
              ) : null}
              {isPending ? (
      ...
      {props.options.map((option) => {
        ...
        return (
          <Pressable
            ...
            accessibilityLabel={localizeMobileString(option.accessibilityLabel ?? option.label)}
