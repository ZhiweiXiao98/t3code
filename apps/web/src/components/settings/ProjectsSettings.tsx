...
function ScopePicker({
  ...
  return (
    ...
    >
      <SelectTrigger
        size="compact"
        aria-label={translate(label === "project" ? "Project scope" : "Machine scope")}
      ...
      >
        <SelectValue className="flex min-w-0 items-center gap-1.5">
          ...
          <span className="truncate">
            {value === null
              ? translate(label === "project" ? "All projects" : "All machines")
              : (selected?.label ??
                translate(label === "project" ? "Unavailable project" : "Unavailable machine"))}
          </span>
        </SelectValue>
      </SelectTrigger>
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="scrollbar-gutter-both shrink-0 overflow-y-auto">
        <WorkspacePageContainer className="pb-0">
          <div
            ...
            role="group"
            aria-label={translate("Settings scope")}
          >
            {environments.length > 3 ? (
              ...
              <ToggleGroup
                aria-label={translate("Machine scope")}
                variant="segmented"
              ...
              >
                <Toggle value="all">{translate("All machines")}</Toggle>
      ...
      {machineId !== null && !machine ? (
        <p className="p-8 text-sm text-muted-foreground">
          {translate("This machine is no longer available.")}
        </p>
      ) : projectKey === null ? (
