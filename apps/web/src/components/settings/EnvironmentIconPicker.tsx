...
export function EnvironmentIconPicker({
  ...
  const automaticLabel =
    detected === null
      ? t("environmentIcon.automatic")
      : t("environmentIcon.automaticDetected", {
          icon: translateWebSource(locale, ENVIRONMENT_MACHINE_KIND_LABELS[detected]),
        });
  ...
  const select = (
    <Select value={value} onValueChange={handleValueChange} disabled={lock !== null}>
      <SelectTrigger size={size} className="w-full sm:w-52" aria-label={t("environmentIcon.label")}>
        <SelectValue>
          <span className="flex min-w-0 items-center gap-2">
            ...
            <span className="truncate">
              {override === null
                ? automaticLabel
                : translateWebSource(locale, ENVIRONMENT_MACHINE_KIND_LABELS[override])}
            </span>
          </span>
        </SelectValue>
      </SelectTrigger>
        ...
        {ENVIRONMENT_MACHINE_KINDS.map((kind) => (
          <SelectItem hideIndicator key={kind} value={kind}>
            <span className="flex min-w-0 items-center gap-2">
              ...
              <span className="truncate">
                {translateWebSource(locale, ENVIRONMENT_MACHINE_KIND_LABELS[kind])}
              </span>
            </span>
          </SelectItem>
        ))}
  ...
  return (
    <Tooltip>
      ...
      <TooltipPopup side="top" className="max-w-72">
        {translateWebSource(locale, lock)}
      </TooltipPopup>
    </Tooltip>
  );
}
