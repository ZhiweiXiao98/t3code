...
export function SettingsSidebarNav({ pathname }: { pathname: string }) {
  ...
  const localizedSearchableItems = useMemo(
    () =>
    ...
    [locale, searchableItems],
  );
  const results = useMemo(
    () => searchSettings(query, localizedSearchableItems),
    [localizedSearchableItems, query],
  );
              ...
              {SETTINGS_NAV_ITEMS.map((item) => {
                ...
                return (
                  <SidebarMenuItem key={item.to}>
                    ...
                    >
                      <Icon />
                      <span className="truncate">{translateWebSource(locale, item.label)}</span>
                    </SidebarMenuButton>
                          ...
                          <SidebarMenuSubItem key={section.targetId}>
                            ...
                            >
                              <span className="ms-0.5">
                                {translateWebSource(locale, section.label)}
                              </span>
                            </SidebarMenuSubButton>
                          </SidebarMenuSubItem>
                        ))}
