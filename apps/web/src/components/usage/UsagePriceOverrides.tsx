...
export function UsagePriceOverrides({
  ...
  for (const draft of stagedDrafts) {
    ...
    if (draft.model.trim() === "") errors.set(draft.id, translate("Enter a model ID."));
    else if (
      attempt === null &&
      (customModels.includes(draft.model.trim()) ||
        drafts.some((other) => other.id !== draft.id && other.model.trim() === draft.model.trim()))
    )
      errors.set(draft.id, translate("This model already has a row. Edit its prices there."));
  }
  ...
  const selectionLabel = selectedIds === null ? translate("All environments") : destinationLabel;
  const discard = () => {
    setDrafts([]);
    setAttempt(null);
  };
      ...
      <DialogPopup className="max-w-3xl">
        ...
        <DialogPanel className="grid gap-4">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <span className="text-xs text-muted-foreground">
              {translate("USD / million tokens")}
            </span>
          </div>
          {selected.length === 0 ? (
            <p className="text-sm text-muted-foreground">
              {environments.length === 0
                ? translate("Connect an environment to set model prices.")
                : translate("Select an environment to see and change its model prices.")}
            </p>
          ) : (
            <>
              <div className="min-w-0 overflow-hidden rounded-lg border border-border">
                <Table
                  ...
                  aria-label={translate("Custom model prices")}
                >
                  <colgroup>
                    <col className="w-[34%]" />
                    ...
                    <TableRow className="hover:bg-transparent">
                      ...
                      <TableHead className="px-1">
                        <Button
                          size="icon-xs"
                          variant="ghost"
                          aria-label={translate("Add model price")}
                          disabled={locked}
                          onClick={() => {
                        ...
                        >
                          <TableCell className="pl-3 whitespace-normal">
                            {row.isNew ? (
                              <Input
                                ...
                                aria-label={translate("New model ID")}
                                aria-invalid={
                            ...
                            (row.model.trim() !== "" ||
                              ...
                              <p role="alert" className="mt-1 text-xs text-destructive">
                                {(() => {
                                  ...
                                  if (!error || locale !== "zh-CN") return error;
                                  const required = /^(.+) is required on (.+)\.$/u.exec(error);
                                  return required
                                    ? `${translate(required[1]!)}是 ${required[2]} 上的必填项。`
                                    : translate(error);
                                })()}
                              </p>
                            ) : null}
                              ...
                              return (
                                <TableCell key={field.key} className="px-1">
                                  <Input
                                    ...
                                    aria-label={
                                      locale === "zh-CN"
                                        ? `${row.model || "新模型"}的${translate(field.label)}价格`
                                    ...
                                    placeholder={
                                      row.isNew
                                        ? field.optional
                                          ? translate("Input rate")
                                          : "0.00"
                                        : translate(cell.placeholder)
                                    }
                                    autoComplete="off"
                                    disabled={locked}
                          ...
                          <TableCell className="px-1">
                            <Tooltip>
                              <TooltipTrigger
                                ...
                                aria-label={
                                  row.removed
                                    ...
                                    : row.isNew
                                      ? translate("Remove new model")
                                      : locale === "zh-CN"
                                        ? `将 ${row.model} 的价格恢复为自动定价`
                              ...
                              <TooltipPopup>
                                {row.removed
                                  ? translate("Undo reset")
                                  : row.isNew
                                    ? translate("Remove row")
                                    : translate("Reset to automatic")}
                              </TooltipPopup>
                            </Tooltip>
                          </TableCell>
                        </TableRow>
              ...
              {attempt.destinations.map((destination) => {
                ...
                return (
                  <div key={destination.environmentId} className="flex justify-between gap-3">
                    ...
                    >
                      {result?.status === "failed"
                        ? `${translate("Not saved")} · ${result.error}`
                        : result?.status === "saved"
                          ? translate("Saved")
                          : translate("Saving…")}
                    </span>
        ...
        <DialogFooter variant="bare" className="items-center sm:justify-between">
          <span className="text-xs text-muted-foreground">
            {hasChanges
              ? locale === "zh-CN"
                ? `更改将应用于 ${destinationLabel}`
                : `Changes apply to ${destinationLabel}`
              : ""}
          </span>
