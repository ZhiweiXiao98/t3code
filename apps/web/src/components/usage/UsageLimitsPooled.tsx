...
function AccountChip({ email }: { readonly email: string }) {
  ...
  return (
    <span
      role="img"
      aria-label={t("usage.limits.account", { account: accountInitials(email) })}
  ...
  return (
    <div className="flex w-72 max-w-[calc(100vw-3rem)] flex-col gap-2.5 text-xs">
      ...
      <div className="flex flex-col gap-1 border-t border-border/60 pt-2.5">
        {account.plan ? <Row label={t("usage.limits.plan")}>{account.plan}</Row> : null}
        {where ? (
          <Row
            label={
              account.environments.length > 0 ? t("usage.limits.signedIn") : t("usage.limits.via")
            }
          >
            {where}
          </Row>
      ...
      <div className="flex flex-col gap-1 border-t border-border/60 pt-2.5">
        <Row label={t("usage.limits.left")}>{remaining}%</Row>
        {window.resetsAt ? (
          <Row label={t("usage.limits.resetsLabel")}>
            {formatUpcomingTimestamp(window.resetsAt, timestampFormat, now)}
            {resetsIn ? ` · ${resetsIn}` : ""}
          </Row>
        ) : null}
        {reset && reset.restoresPercent > 0 ? (
          <Row label={t("usage.limits.restores")}>
            {t("usage.limits.restoresPool", { percent: reset.restoresPercent })}
          </Row>
        ) : null}
      </div>
      {credits && redeem ? (
        <div className="border-t border-border/60 pt-2.5 text-muted-foreground">
          <span className="flex items-center gap-3">
            <span className="tabular-nums">{resetCreditsSummary(credits, now, t, true)}</span>
            <Button
              size="xs"
              variant="outline"
              ...
              onClick={onRedeem}
            >
              {redeem.busy ? t("usage.limits.usingCredit") : t("usage.limits.useReset")}
            </Button>
          </span>
        </div>
      ) : null}
...
function PoolSegment({
  ...
  const accountLabel =
    account.displayName ?? (account.email ? accountInitials(account.email) : account.driver);
  const summary = [
    accountLabel,
    t("usage.limits.remaining", { percent: remaining }),
    resetsIn,
    credits === 0
      ? null
      : credits === 1
        ? t("usage.limits.creditBanked")
        : t("usage.limits.creditsBanked", { count: credits }),
  ]
    .filter(Boolean)
    .join(", ");
  return (
  ...
  return (
    ...
    >
      <span className="relative inline-flex size-4 shrink-0 items-center justify-center rounded-sm text-[10px] leading-none font-semibold text-foreground/80 tabular-nums">
        ...
        />
        <span className="sr-only">{t("usage.limits.segment")} </span>
