    ...
    <div className="flex max-h-[calc(var(--available-height)-0.5rem)] min-h-0 w-fit max-w-[min(24rem,calc(100vw-2rem))] flex-col text-left">
        ...
        {state.status === "available" ? (
          <div>
            <div className="whitespace-nowrap text-sm leading-5 font-medium">
              {t("sidebar.update.readyToDownload")}
            </div>
      ...
      <div className="min-h-0 max-h-[min(28rem,calc(100vh-6rem))] overflow-y-auto px-1 pt-4 pb-1">
        {state.releaseNotes.map((releaseNote, index) => {
          ...
          const linkLabel =
            omittedItemCount === 0
              ? t("sidebar.update.viewRelease")
              : omittedItemCount === 1
                ? t("sidebar.update.moreChange", { count: omittedItemCount })
                : t("sidebar.update.moreChanges", { count: omittedItemCount });
          ...
          <div>
            ...
            <ReleaseLink releaseUrl={getDesktopUpdateReleaseHistoryUrl()} shell={shell}>
              {state.omittedReleaseCount === 1
                ? t("sidebar.update.olderRelease", { count: state.omittedReleaseCount })
                : t("sidebar.update.olderReleases", { count: state.omittedReleaseCount })}
            </ReleaseLink>
          </div>
        ) : null}
      </div>
