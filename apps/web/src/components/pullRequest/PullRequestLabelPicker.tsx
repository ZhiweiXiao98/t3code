...
export function PullRequestLabelPicker({
    ...
    if (result._tag === "Failure") {
      toastManager.add({
        ...
        title: candidate.isApplied
          ? t("pullRequests.labels.removeFailed", { label: candidate.name })
          : t("pullRequests.labels.addFailed", { label: candidate.name }),
        description: readableFailure(
          squashAtomCommandFailure(result),
          t("pullRequests.labels.permissionFailure"),
        ),
      });
      return;
    }
  ...
  return (
    <PullRequestCandidatePicker
      ...
      label={t("pullRequests.labels.change")}
      allowed={allowed}
      disabledReason={t("pullRequests.labels.needsTriage")}
      open={open}
      ...
      errorLabel={t("pullRequests.labels.readFailed")}
      truncated={candidatesQuery.data?.truncated === true}
      truncatedLabel={t("pullRequests.labels.truncated")}
      ...
      {(candidate) => {
        ...
        return (
          <>
            ...
            {candidate.isApplied ? (
              <CheckIcon
                aria-label={t("pullRequests.labels.applied")}
