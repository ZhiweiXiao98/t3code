...
function PullRequestFilterRadioGroup<Value extends string>({
  ...
  return (
    ...
    >
      <MenuGroupLabel>{translateWebSource(locale, label)}</MenuGroupLabel>
      {options.map((option) => {
          ...
          >
            <span className="flex min-w-0 items-center gap-2">
              ...
              <span className="min-w-0 flex-1 truncate">
                {translateWebSource(locale, option.label)}
              </span>
              {option.unavailable ? (
                <span className="shrink-0">· {t("pullRequests.filter.unavailable")}</span>
              ) : null}
            </span>
          </MenuRadioItem>
        );
  ...
  return (
    <MenuSub>
      <MenuSubTrigger>
        ...
        <span className="flex-1">{translateWebSource(locale, label)}</span>
  ...
  return (
    <MenuSub>
      <MenuSubTrigger>
        ...
        <span className="flex-1">{t("pullRequests.filter.author")}</span>
        <span className="min-w-0 max-w-32 truncate text-xs text-muted-foreground">
        ...
        <div className="p-1 pb-2">
          <InputGroup>
            ...
            <InputGroupInput
              ...
              placeholder={t("pullRequests.filter.searchAuthors")}
              aria-label={t("pullRequests.filter.searchAuthors")}
            />
          </InputGroup>
        </div>
            ...
            <MenuRadioItem key={option.actor.login.toLowerCase()} value={option.actor.login}>
              <span className="flex min-w-0 items-center gap-2">
                ...
                <span className="shrink-0 text-xs text-muted-foreground tabular-nums">
                  {t("pullRequests.filter.mergesLoaded", { count: option.mergedCount })}
                </span>
              </span>
            </MenuRadioItem>
          ))}
          {visible.length === 0 ? (
            <MenuItem disabled>{t("pullRequests.filter.noAuthors")}</MenuItem>
          ) : null}
        </MenuRadioGroup>
      </MenuSubPopup>
    </MenuSub>
  ...
  return (
    <MenuSub>
      <MenuSubTrigger>
        ...
        <span className="flex-1">{t("pullRequests.filter.labels")}</span>
        <span className="text-xs text-muted-foreground">
          {value.length === 0
            ? t("pullRequests.filter.any")
...
export function PullRequestFiltersMenu({
  ...
  const projectOptions: ReadonlyArray<PullRequestFilterOption<string>> = [
    { value: ALL_PROJECTS_VALUE, label: t("pullRequests.filter.allProjects"), Icon: LayersIcon },
    ...projects
      .toSorted(
        (left, right) =>
