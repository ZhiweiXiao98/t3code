...
function PullRequestsRouteView() {
  ...
  const panelToggleControls = (
    <PanelLayoutControls
      ...
      rightPanelUnavailableLabel={t("pullRequests.panel.selectFirst")}
      liveAgentCount={0}
    ...
    <>
      ...
      {listQuery.error && entries.length > 0 ? (
        <div className="flex items-center justify-between gap-3 rounded-lg border border-amber-500/30 bg-amber-500/5 px-3 py-2 text-xs">
          <span>
            {listQuery.error} {t("pullRequests.latestLoaded")}
          </span>
        ...
        <div className="flex justify-center py-3 text-xs text-muted-foreground">
          {loadingMore ? (
            <span className="flex items-center gap-2">
              ...
              {t(sentCursors === null ? "pullRequests.updating" : "pullRequests.loadingMore")}
            </span>
            ...
            >
              {t("pullRequests.empty.loadMore")}
            </Button>
          ) : (
            <span>{t("pullRequests.narrowSearch")}</span>
          )}
        </div>
      ) : null}
    </>
  ...
  const sortMenu = (
    <CompactFilterMenu
      label={translateWebSource(locale, "Sort pull requests")}
      triggerIcon={<ArrowDownUpIcon aria-hidden className="size-4" />}
      triggerLabel={translateWebSource(locale, "Sort")}
      outlined
      value={sort}
  ...
  return (
    <Menu>
      <MenuTrigger
        aria-label={
          triggerLabel
            ? `${translateWebSource(locale, label)}: ${translateWebSource(locale, current.label)}`
      ...
      >
        {triggerLabel ? (
          <>
            {triggerIcon}
            <span>{translateWebSource(locale, triggerLabel)}</span>
          </>
        ) : (
          <>
            <span className="truncate">{translateWebSource(locale, current.label)}</span>
  ...
  return (
    ...
    <div className="@container/pr-list flex min-h-0 min-w-0 flex-1 flex-col bg-background">
      ...
      >
        {titlebarControls}
        {condensed ? (
          <WorkspaceBreadcrumb ariaLabel={t("pullRequests.scope")} className="overflow-hidden">
